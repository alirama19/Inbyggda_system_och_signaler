function [h1,h2,t,u,e] = vm_kaskad(a,N,dT,v, kp, Ti, Td)
...
for i=1:N %slinga kommer att köras N-gångar, varje gång tar exakt Ts-sekunder
    ...
    % REGULATORN
    % PI- som yttre reglering i kaskad-reglering
    if ((mod(i,5)==0)|| 1==1)
       errorsum=(e(i)+errorsum);
        u(i)=kp*(e(i) + (errorsum * (dT/Ti)));
         pi_u = u(i);
    else
        u(i)=0; % Ingen förändring
         pi_u = u(i);
    end   
    
    e(i) = pi_u - h1(i);
   % PD- som inre reglering i kaskad-reglering
   if(e(i)>0 && i > 1)
         u(i)=kp*(e(i) + Td * ((e(i)-e(i-1))/dT)); % ny styrsignal
    else
       u(i)=0; % ingen förändring
    end
...
title('Kaskad-reglering vattenmodell')
legend('h1 ', 'h2 ', 'u ', 'e')
