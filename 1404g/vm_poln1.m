function [h1,h2,t,u,e] = vm_poln1(a,N,dT,v,Kp) %funktionen reglerar nivån i första behållaren.
...
Pz = 1;
Bz = (Kp*dT)/TankArean;
Az = 1 + ( (areanAvUtflode*dT*sqrt(2*gravity)) / (TankArean*2*sqrt(n1)) - 1 );
Kr = Pz / Bz;
Cz = 1;
Dz = (Pz-Az) / Bz;

  e(i)= ((borvarde+Kr-Dz)*Cz)-h1(i); %räknar ut felvärdet som differens mellan ärvärdet och börvärdet
...
% DEL F: avsluta experimentet
  ...
  title('Deadbeat-reglering tank1')
  legend('h1 ', 'h2 ', 'u ','e')
